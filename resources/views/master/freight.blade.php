...
<section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>List Freight Charge</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ url('/dashboard') }}">Home</a></li>
            <li class="breadcrumb-item active">list Freight Charge</li>
          </ol>
        </div>
      </div>
    </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="{{ '/master/freight_delete/'.$list->id }}" class="btn btn-danger btn-xs hapus-link"><i class="fa fa-trash"></i> Delete </a>
                                </td>
                            </tr>
                            @endforeach
                    </table>
    ...
    <script>
        ...
        function simpandata(){
            if($.trim($("#freight").val()) == ""){
                Swal.fire({
                    title: 'Error!',
                    text: 'Please enter Freight Charge!',
                    icon: 'error'
                })
            }else{
                if(dsState=="Input"){                
                    $('#formku').attr("action", "{{ route('master.freight_doAdd') }}");
                    $('#formku').submit();              
                }else{
                    $('#formku').attr("action", "{{ route('master.freight_doEdit') }}");
                    $('#formku').submit(); 
                }
            }
        ...
        function editData(id){
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('master.freight_get') }}",
                dataType: 'json',
                data : {id: id},
                success: function (result){
                    $('#id').val(result.id);
                    $('#freight').val(result.freight_charge);
