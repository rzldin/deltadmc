...
<section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>List Incoterms</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ url('/dashboard') }}">Home</a></li>
            <li class="breadcrumb-item active">list Incoterms</li>
          </ol>
        </div>
      </div>
    </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="{{ '/master/incoterms_delete/'.$list->id }}" class="btn btn-danger btn-xs hapus-link"><i class="fa fa-trash"></i> Delete </a>
                                </td>
                            </tr>
                            @endforeach
                    </table>
                ...
                <form class="eventInsForm" method="post" target="_self" name="formku" 
                    ...
                    <div class="row">
                        <div class="col-md-4 col-xs-4">
                           Incoterms Code<font color="#f00">*</font>
                        </div>
                        <div class="col-md-8 col-xs-8">
                            <textarea name="code" id="code" rows="3" class="form-control" placeholder="Input Incoterms Code ..." onkeyup="this.value = this.value.toUpperCase()"></textarea>
    ...
    <script>
        ...
        function simpandata(){
            if($.trim($("#code").val()) == ""){
                Swal.fire({
                    title: 'Error!',
                    text: 'Please enter Incoterms Code!',
                    icon: 'error'
                })
            }else{
                if(dsState=="Input"){                
                    $('#formku').attr("action", "{{ route('master.incoterms_doAdd') }}");
                    $('#formku').submit();              
                }else{
                    $('#formku').attr("action", "{{ route('master.incoterms_doEdit') }}");
                    $('#formku').submit(); 
                }
            }
        ...
        function editData(id){
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('master.incoterms_get') }}",
                dataType: 'json',
                data : {id: id},
                success: function (result){
                    $('#id').val(result.id);
                    $('#code').val(result.incoterns_code);
